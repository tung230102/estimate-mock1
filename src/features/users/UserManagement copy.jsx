...
import { createUser, removeUser, getListUsers, updateUser } from "~/api";
import {
  CommonButton,
  ...
  const handleCreateUser = (data) => {
    createUser(data).then((res) => {
      if (res?.statusCode === statusCode.CREATED) {
        showToast("Create new user success!");
        queryListUser();
  ...
  const handleUpdateUser = (data) => {
    updateUser(userToUpdate.id, data).then((res) => {
      if (res?.statusCode === statusCode.OK) {
        showToast("Update new user success!");
        queryListUser();
        setShowModalCreate(false);
        setUserToUpdate({});
      } else if (res?.statusCode === statusCode.BAD_REQUEST) {
